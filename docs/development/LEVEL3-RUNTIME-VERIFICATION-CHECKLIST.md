...
- 需要两批验证材料：
  - **单玩家验证**（一人即可）：启动、落盘、重启恢复、命令、央行真机、
    紧急权限、客户端展示面、土地认领 inspect/claim、信函收发/读取/删除。
...
使用带 FontaineRepublic 模组的 Forge 客户端连接（协议 **v8**）：

- 服务端日志出现 `Channel fontainerepublic:main protocol 8 frozen with 27
...
- 旧协议（v7-）模组客户端被握手拒绝（协议 v8，仅接受原样匹配或 ABSENT）；
- 登出后客户端展示缓存清空（重进不显示旧数据）。

**通过条件：** 握手、登录同步、全 11 个 `/frclient` 视图（含 court/land）、
通信器持有门禁、交易通知、无客户端平行性、登出清理全部成立。
...
客户端展示面：通过/不通过/未做（协议 v8/27 消息、11 个 /frclient 视图、通信器门禁、
  no-client 平行性、旧协议拒绝）
