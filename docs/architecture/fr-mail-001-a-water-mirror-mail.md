...
### 6.6 机构一键向全体公民发函（Human 需求 2026-08-14）

- **广播邮件**：仅授权机构身份可发起（部门职位持有人 / 议会/法院/央行邮箱
  管理员 / 水神 / 控制台）；普通玩家不可群发；
- **投递**：广播邮件在 mail 命名空间**只存一份**；每个公民收件箱视图派生
  包含该广播，未读状态按人记录（有界已读集合，防重复提醒）；
- **收件范围**：全体公民 = 有公民记录且 status=CITIZEN 的主体；内部有界
  枚举（不暴露公开 API）；
- **费用**：机构发件免费 → 广播免费（可配 `mail.broadcastFee`，默认 0）；
- **防滥用**：广播冷却（可配 `mail.broadcastCooldownMillis`）+ 仅授权身份；
- 协议：C2S `MailBroadcastPacket`（subject≤64/body≤2000），S2C 随
  MailboxSync 派生展示 + MailAlert 未读提醒；
- 测试：授权矩阵（水神/控制台/负责人/普通玩家拒绝）、冷却、已读状态、
  离线玩家登录后可见、有界。
