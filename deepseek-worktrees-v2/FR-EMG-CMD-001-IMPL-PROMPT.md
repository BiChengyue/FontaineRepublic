...
- 命令面：`preview` / `confirm` / `inspect` / `status` / `bootstrap` /
  `stage` / `recover`（权威配置命令仅做适配与有界反馈，配置本体仍归 FR-EMG 服务）。
- 只实现本任务；不实现业务动作、token/日志/权威配置本体、GUI、包。
...
- 必读（按顺序）：运行手册 -> CLAUDE.md -> current_status.md ->
  `docs/architecture/fr-emg-001-a-hydro-archon-emergency-authority.md`（§4/§13/§14）->
  ...
  CommandRuntimeResolver / CommandFeedback / CommandContributionRegistry）->
  `src/main/java/com/fontainerepublic/server/emergency/api/EmergencyService.java`
  （preview/confirm/inspect/status/bootstrapAuthority/stageAuthority/recoverAuthority
  与结果类型 PreviewResult/ConfirmResult/EmergencyInspection/EmergencyStatus/
  ConfigureResult）。

## 3. 执行约束（违反即失败）

- 只在工作树内修改；不推送/SSH；不修改设计文档与台账；
- 命令适配只做解析/来源映射/运行时解析与有界反馈；actor/console/token/日志/
  权威配置生命周期归 FR-EMG 服务（不复制）；
- 每次执行解析当前 ACTIVE EmergencyService，不捕获；
- token 只展示一次、绝不落日志；输出有界；非法输入 fail closed；
- bootstrap/recover 仅接受 LOCAL_CONSOLE 来源（由服务端核验）；stage 接受
  已授权水神或控制台；
- 不新增依赖；开始时报分支/HEAD/工作区状态。

## 4. 验收（FR-EMG-001-A §4/§13/§14）

- `preview/confirm/inspect/status/bootstrap/stage/recover` 命令树可构建、
  字面量与参数正确；
- 来源映射：LOCAL_CONSOLE→SERVER_CONSOLE；PLAYER→HYDRO_ARCHON；
  RCON/命令方块/函数/集成主机到达回调也被服务拒绝；
- 运行时解析不捕获；无服务时 fail closed；
- bootstrap/stage/recover 输出 ConfigureResult 有界摘要；
